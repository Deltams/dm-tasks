...
def mack_sfp():
    ...
    attempts_label.configure(text=f'Осталось попыток: {attempts}')
...
greet_label = tk.Label(top_frame, text='Ваш вектор:', font=('Arial', 16, 'normal'))
greet_label.pack()

vector_label = tk.Label(top_frame, text='', font=('Arial', 16, 'normal'))
vector_label.pack()

error_label = tk.Label(top_frame, text='', font=('Arial', 14, 'normal'), fg='#ff4000')
error_label.pack()

top_frame.pack(side='top')

attempts_label = tk.Label(mid_frame, text='Осталось попыток: 999', font=('Arial', 14, 'normal'))
attempts_label.pack(side='left', padx=10)

user_entry = tk.Entry(mid_frame, font=('Arial', 14, 'normal'))
user_entry.pack(side='left', padx=10)

button_submit = tk.Button(mid_frame, text='Ответить', font=('Arial', 12, 'normal'), bg='#bfbfbf', command=send_vector)
button_submit.pack(side='left', padx=10)

mid_frame.pack(side='left', padx=0)

button_help = tk.Button(bottom_frame, text='Справка', font=('Arial', 8, 'normal'), bg='#bfbfbf', command=open_child_root)
